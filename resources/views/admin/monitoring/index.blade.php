    ...
    <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
        ...
        <div class="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200/80 dark:border-slate-800 shadow-sm p-5">
            <div class="flex items-center justify-between mb-4">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p class="text-[11px] {{ $statusLabel['text'] }} font-medium flex items-center gap-1.5">
            ...
            <dl class="space-y-2 text-xs">
                ...
                <div class="flex justify-between"><dt class="text-slate-500 dark:text-slate-400">Temps de réponse</dt><dd class="font-medium text-slate-700 dark:text-slate-200">{{ $s['response'] }}</dd></div>
                ...
                <div class="flex justify-between"><dt class="text-slate-500 dark:text-slate-400">Dernière sync</dt><dd class="font-medium text-slate-700 dark:text-slate-200">{{ $s['last_sync'] }}</dd></div>
                @if($s['last_error'])
                <div class="pt-2 border-t border-slate-100 dark:border-slate-800">
                    <dt class="text-slate-500 dark:text-slate-400 mb-1">Dernière erreur</dt>
                    <dd class="text-[11px] text-red-600 dark:text-red-400 font-mono break-all">{{ $s['last_error'] }}</dd>
                </div>
                @endif
            </dl>
        </div>
