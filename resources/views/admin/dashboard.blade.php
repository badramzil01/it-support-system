...
<div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
    <div>
        <p class="text-xs font-medium text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-0.5">{{ now()->locale('fr')->isoFormat('dddd D MMMM YYYY') }}</p>
        <h2 class="brand-font text-2xl font-bold text-slate-900 dark:text-white">Bonjour, {{ auth()->user()->name ?? 'Admin' }} 👋</h2>
        <p class="text-sm text-slate-500 dark:text-slate-400 mt-1">Voici la vue d'ensemble de la plateforme Support IT.</p>
    </div>
    <div class="flex items-center gap-2 shrink-0">
        <a href="{{ route('admin.ui.tickets.index') }}" class="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-amber-600 hover:bg-amber-700 text-white text-sm font-semibold shadow-sm shadow-amber-500/30 transition">
        ...
        <a href="{{ route('admin.ui.users.index') }}" class="inline-flex items-center gap-2 px-4 py-2 rounded-xl border border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-800 text-slate-700 dark:text-slate-200 text-sm font-semibold transition">
...
<div class="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4 mb-6">
    ...
    <div class="stat-card bg-white dark:bg-slate-900 rounded-2xl border border-slate-200/80 dark:border-slate-800 p-4 md:p-5 shadow-sm">
        <div class="flex items-start justify-between mb-3">
            <div class="flex h-10 w-10 items-center justify-center rounded-xl {{ $k['bg'] }}">
                <svg class="h-5 w-5 {{ $k['color'] }}" viewBox="0 0 20 20" fill="currentColor"><path fill-rule="evenodd" d="{{ $k['icon'] }}" clip-rule="evenodd"/></svg>
            </div>
            @if($k['badge'])
            <span class="inline-flex items-center gap-1 text-[10px] font-semibold px-2 py-0.5 rounded-full {{ $k['bc'] }}">{{ $k['badge'] }}</span>
            @endif
        </div>
        <p class="text-3xl font-bold text-slate-900 dark:text-white leading-none">{{ $k['value'] }}</p>
        <p class="text-xs font-medium text-slate-500 dark:text-slate-400 mt-1.5">{{ $k['title'] }}</p>
        <div class="mt-3 h-1 bg-slate-100 dark:bg-slate-800 rounded-full overflow-hidden">
            <div class="h-full {{ $k['pc'] }} rounded-full" style="width: {{ min(100, max(0, $k['progress'])) }}%"></div>
        </div>
    </div>
    @endforeach
</div>
...
<div class="grid grid-cols-1 xl:grid-cols-3 gap-4 md:gap-5 mb-6">
    <div class="xl:col-span-2 bg-white dark:bg-slate-900 rounded-2xl border border-slate-200/80 dark:border-slate-800 shadow-sm p-5">
        <div class="flex items-center justify-between mb-4">
            ...
            <span class="text-[10px] font-semibold text-slate-500 dark:text-slate-400 uppercase">Total : {{ array_sum($ticketsPerDayData) }}</span>
        </div>
...
<div class="grid grid-cols-1 xl:grid-cols-3 gap-4 md:gap-5 mb-6">
    ...
    <div class="xl:col-span-2 bg-white dark:bg-slate-900 rounded-2xl border border-slate-200/80 dark:border-slate-800 shadow-sm overflow-hidden">
        <div class="flex items-center justify-between px-5 py-4 border-b border-slate-100 dark:border-slate-800">
            ...
            <a href="{{ route('admin.ui.tickets.index') }}" class="text-xs font-medium text-amber-600 dark:text-amber-400 hover:underline">Voir tout →</a>
        </div>
        <div class="divide-y divide-slate-100 dark:divide-slate-800">
            ...
            <div class="flex items-center gap-3 px-5 py-3 hover:bg-slate-50 dark:hover:bg-slate-800/40 transition">
                <span class="h-2 w-2 shrink-0 rounded-full {{ match($t->priority) { 'critical'=>'bg-red-500', 'high'=>'bg-orange-500', 'medium'=>'bg-amber-400', default=>'bg-slate-300' } }} dark:{{ match($t->priority) { 'critical'=>'bg-red-400', 'high'=>'bg-orange-400', 'medium'=>'bg-amber-300', default=>'bg-slate-600' } }}"></span>
                <div class="min-w-0 flex-1">
                    <div class="flex items-center gap-2 min-w-0">
                        <span class="text-xs font-mono font-medium text-slate-400 shrink-0">#{{ $t->id }}</span>
                        <p class="text-sm font-medium text-slate-800 dark:text-slate-200 truncate">{{ Str::limit($t->title, 50) }}</p>
                    </div>
                    <div class="flex items-center gap-3 mt-0.5">
                        <span class="text-[11px] text-slate-400">{{ optional($t->user)->name ?? '—' }}</span>
                ...
                <span class="badge shrink-0 {{ match($t->status) { 'resolved'=>'bg-emerald-50 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-400', 'in_progress'=>'bg-violet-50 text-violet-700 dark:bg-violet-900/30 dark:text-violet-400', 'closed'=>'bg-slate-50 text-slate-500 dark:bg-slate-800 dark:text-slate-400', default=>'bg-blue-50 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400' } }}">{{ ucfirst(str_replace('_',' ',$t->status)) }}</span>
            </div>
            @empty
...
const ticketsPerDayLabels = @json($ticketsPerDayLabels);
const ticketsPerDayData = @json($ticketsPerDayData);
