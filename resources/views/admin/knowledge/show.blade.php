    ...
    <div class="grid grid-cols-1 xl:grid-cols-3 gap-4">
        <div class="xl:col-span-2 space-y-4">
            ...
            <div class="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200/80 dark:border-slate-800 shadow-sm overflow-hidden">
                ...
                <div class="divide-y divide-slate-100 dark:divide-slate-800">
                    ...
                    <a href="{{ route('admin.ui.knowledge.show', $r) }}" class="flex items-start gap-3 px-5 py-3 hover:bg-slate-50 dark:hover:bg-slate-800/40 transition">
                        <div class="flex h-8 w-8 items-center justify-center rounded-lg bg-amber-50 dark:bg-amber-900/30 shrink-0">
        ...
        <div class="space-y-4">
            ...
            <div class="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200/80 dark:border-slate-800 shadow-sm p-5">
                ...
                <a href="{{ route('admin.ui.knowledge.author', $item->author->id) }}" class="flex items-center gap-3 group">
                    <div class="flex h-12 w-12 shrink-0 items-center justify-center rounded-full bg-gradient-to-br from-amber-500 to-orange-600 text-base font-bold text-white ring-2 ring-amber-200 dark:ring-amber-900">
                        {{ Str::upper(Str::substr($item->author->name, 0, 1)) }}
                    </div>
                    <div class="min-w-0 flex-1">
                        <p class="text-sm font-semibold text-slate-900 dark:text-white group-hover:text-amber-600 dark:group-hover:text-amber-400 transition truncate">{{ $item->author->name }}</p>
                        <p class="text-xs text-slate-500 truncate">{{ $item->author->email }}</p>
                        <p class="text-[10px] text-slate-400 mt-0.5">Support</p>
                    </div>
                    <svg class="h-4 w-4 text-slate-400 group-hover:text-amber-600 transition" viewBox="0 0 20 20" fill="currentColor"><path fill-rule="evenodd" d="M7.21 14.77a.75.75 0 0 1 .02-1.06L11.168 10 7.23 6.29a.75.75 0 1 1 1.04-1.08l4.5 4.25a.75.75 0 0 1 0 1.08l-4.5 4.25a.75.75 0 0 1-1.06-.02Z" clip-rule="evenodd"/></svg>
                </a>
            </div>
            @endif
            ...
            <div class="flex flex-wrap gap-2">
                <button onclick="openKbModal(@json($item))" class="flex-1 inline-flex items-center justify-center gap-1.5 rounded-lg bg-amber-600 px-3 py-2 text-xs font-semibold text-white hover:bg-amber-700 transition">
                ...
                <form method="POST" action="{{ route('admin.ui.knowledge.destroy', $item) }}" onsubmit="return confirm('Supprimer ?')" class="flex-1">
                    @csrf @method('DELETE')
