...
<head>
    ...
    <title>@yield('title', 'Dashboard') — Support IT</title>
...
<div id="cmdPalette" class="fixed top-[15%] left-1/2 -translate-x-1/2 z-50 w-full max-w-xl pointer-events-none opacity-0 scale-95">
    <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl border border-slate-200 dark:border-slate-700 overflow-hidden">
        ...
        <div class="p-2 max-h-80 overflow-y-auto">
            ...
            <a href="{{ route('support.dashboard') }}" class="flex items-center gap-3 px-3 py-2 rounded-lg hover:bg-slate-50 dark:hover:bg-slate-700/60 text-sm text-slate-700 dark:text-slate-200 transition">
            ...
            <a href="{{ route('support.ui.tickets.index') }}" class="flex items-center gap-3 px-3 py-2 rounded-lg hover:bg-slate-50 dark:hover:bg-slate-700/60 text-sm text-slate-700 dark:text-slate-200 transition">
            ...
            <a href="{{ route('support.ui.knowledge.index') }}" class="flex items-center gap-3 px-3 py-2 rounded-lg hover:bg-slate-50 dark:hover:bg-slate-700/60 text-sm text-slate-700 dark:text-slate-200 transition">
        ...
        <nav class="flex-1 overflow-y-auto px-2 py-3 space-y-5">
            ...
            <div>
                ...
                <a href="{{ route('support.dashboard') }}"
                   class="nav-item {{ request()->routeIs('support.dashboard') ? 'active' : '' }} flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium text-slate-400 hover:text-white transition">
            ...
            <div>
                ...
                {{-- DISABLED: Escalated Tickets merged into single Tickets page
                <a href="{{ route('support.ui.escalations.index') }}"
                   class="nav-item {{ request()->routeIs('support.ui.escalations*') ? 'active' : '' }} flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium text-slate-400 hover:text-white transition">
                    <svg class="h-4 w-4 shrink-0" viewBox="0 0 20 20" fill="currentColor">
            ...
            <div>
                ...
                <a href="{{ route('support.ui.knowledge.index') }}"
                   class="nav-item {{ request()->routeIs('support.ui.knowledge.*') ? 'active' : '' }} flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium text-slate-400 hover:text-white transition">
                    <svg class="h-4 w-4 shrink-0" viewBox="0 0 20 20" fill="currentColor">
            ...
            <div>
                <p class="px-3 mb-1 text-[10px] font-semibold uppercase tracking-widest text-slate-600">Configuration</p>
                     <a href="{{ route('support.ui.notifications.index') }}#"
        ...
        <div class="shrink-0 p-3 border-t border-slate-800/80">
            <div class="flex items-center gap-2.5 px-2.5 py-2 rounded-xl hover:bg-slate-800/60 transition cursor-pointer group">
                ...
                <div class="min-w-0 flex-1">
                    <p class="truncate text-xs font-semibold text-slate-200">{{ auth()->user()->name ?? 'Agent' }}</p>
                    <p class="truncate text-[10px] text-slate-500">{{ auth()->user()->email ?? 'support@it.com' }}</p>
                </div>
            ...
            <div class="ml-auto flex items-center gap-1.5">
                ...
                <div class="relative">
                    <button onclick="toggleDropdown('notifMenu')"
                        ...
                        @endphp
                        @if($unreadCount > 0)
                            <span id="notifBadge" class="absolute -top-1 -right-1 inline-flex items-center justify-center h-5 min-w-[20px] px-1 text-[11px] font-semibold rounded-full bg-red-500 text-white">{{ $unreadCount }}</span>
                        @else
                    ...
                    <div id="notifMenu" class="dropdown-menu absolute right-0 mt-1.5 w-80 rounded-xl bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 shadow-xl shadow-black/10 overflow-hidden">
                        ...
                        <div class="px-4 py-2 border-t border-slate-100 dark:border-slate-700">
                            <a href="{{ route('support.ui.notifications.index') }}" class="text-xs font-medium text-blue-600 hover:text-blue-700 dark:text-blue-400">Voir tout →</a>
                        </div>
                    </div>
                </div>
                ...
                <div class="relative pl-1.5 border-l border-slate-200 dark:border-slate-700">
                    ...
                    <div id="userMenu" class="dropdown-menu absolute right-0 mt-1.5 w-52 rounded-xl bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 shadow-xl shadow-black/10 overflow-hidden py-1">
                        <div class="px-3 py-2 border-b border-slate-100 dark:border-slate-700 mb-1">
                            <p class="text-xs font-semibold text-slate-800 dark:text-slate-200">{{ auth()->user()->name ?? 'Agent' }}</p>
                            <p class="text-[10px] text-slate-500">{{ auth()->user()->email ?? '' }}</p>
                        </div>
                        <a href="{{ route('support.ui.settings.index') }}" class="flex items-center gap-2.5 px-3 py-2 text-xs text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700/60 transition">
...
(function(){
    async function fetchUnread() {
        try {
            const res = await fetch('{{ url("equipeIT/ui/notifications/unread-count") }}', { headers: { 'Accept': 'application/json' } });
            if (!res.ok) return;
