...
<div class="bg-gradient-to-r from-blue-500 via-indigo-500 to-violet-600 rounded-2xl p-5 mb-6 text-white shadow-lg shadow-blue-500/20">
    <div class="flex items-center gap-4">
        ...
        <a href="{{ route('support.ui.knowledge.author', $stats['top_contributor']->id) }}" class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-white/20 hover:bg-white/30 text-sm font-semibold transition">
            Voir profil
...
<div class="rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-4 mb-4">
    <form method="GET" action="{{ route('support.ui.knowledge.index') }}">
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-6 gap-3">
            <div class="sm:col-span-2">
                ...
                <div class="relative">
                    ...
                    <input type="text" name="search" value="{{ request('search') }}" placeholder="Question, réponse, auteur, email…" class="w-full rounded-lg border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 py-2 pl-9 pr-3 text-sm outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-100 dark:focus:ring-blue-950 placeholder:text-slate-400">
                </div>
            </div>
            <div>
            ...
            <div>
                ...
                <input type="date" name="date_from" value="{{ request('date_from') }}" class="w-full rounded-lg border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 py-2 px-3 text-sm outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-100 dark:focus:ring-blue-950">
            </div>
            <div>
                <label class="block mb-1 text-[11px] font-medium text-slate-500 dark:text-slate-400">Jusqu'au</label>
                <input type="date" name="date_to" value="{{ request('date_to') }}" class="w-full rounded-lg border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 py-2 px-3 text-sm outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-100 dark:focus:ring-blue-950">
            </div>
            <div class="sm:col-span-2 lg:col-span-4 xl:col-span-6 flex flex-wrap items-center justify-end gap-2">
                <a href="{{ route('support.ui.knowledge.index') }}" class="inline-flex items-center gap-1.5 rounded-lg border border-slate-200 dark:border-slate-700 px-4 py-2 text-sm font-medium text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800 transition">Réinitialiser</a>
        ...
        <table class="w-full text-sm">
            ...
            <tbody class="divide-y divide-slate-100 dark:divide-slate-800">
                ...
                <tr class="hover:bg-slate-50 dark:hover:bg-slate-800/40 transition group">
                    ...
                    <td class="px-4 py-3">
                        @if($item->author)
                        <a href="{{ route('support.ui.knowledge.author', $item->author->id) }}" class="flex items-center gap-2 hover:underline">
                            <div class="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-gradient-to-br from-blue-500 to-violet-600 text-[10px] font-bold text-white">{{ Str::upper(Str::substr($item->author->name, 0, 1)) }}</div>
                            <div class="min-w-0">
                    ...
                    <td class="px-4 py-3 whitespace-nowrap text-right">
                        <div class="flex items-center justify-end gap-1">
                            <a href="{{ route('support.ui.knowledge.show', $item) }}" class="inline-flex items-center gap-1 rounded-lg bg-blue-600 px-2.5 py-1.5 text-[11px] font-medium text-white hover:bg-blue-700 transition" title="Voir">
                            ...
                            <form method="POST" action="{{ route('support.ui.knowledge.destroy', $item) }}" onsubmit="return confirm('Confirmer la suppression de cette solution ?')" class="inline">
                                @csrf @method('DELETE')
        ...
        <div class="flex items-center justify-between text-xs text-slate-500 dark:text-slate-400">
            ...
            <div class="flex items-center gap-1">
                ...
                @foreach($items->getUrlRange(max(1,$items->currentPage()-2), min($items->lastPage(),$items->currentPage()+2)) as $page => $url)
                    <a href="{{ $url }}" class="inline-flex h-8 w-8 items-center justify-center rounded-lg border text-xs font-medium transition {{ $page == $items->currentPage() ? 'border-blue-600 bg-blue-600 text-white' : 'border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-800 text-slate-600 dark:text-slate-300' }}">{{ $page }}</a>
                @endforeach
