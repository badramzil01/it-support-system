...
<div class="space-y-4">
    ...
    <div class="rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-3 flex items-center gap-3">
        ...
        <span class="text-xs text-slate-400 ml-2">{{ $tickets->total() }} ticket(s) visible</span>
    </div>
    @endif
        ...
        <form method="GET" action="{{ route('support.ui.tickets.index') }}">
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-6 gap-3">
                ...
                <div>
                    ...
                    <select name="status" class="w-full rounded-lg border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 py-2 px-3 text-sm outline-none focus:border-blue-500">
                ...
                <div>
                    ...
                    <select name="priority" class="w-full rounded-lg border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 py-2 px-3 text-sm outline-none focus:border-blue-500">
                ...
                <div>
                    ...
                    <select name="category" class="w-full rounded-lg border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 py-2 px-3 text-sm outline-none focus:border-blue-500">
                        <option value="">Toutes</option>
                ...
                <div>
                    <label class="block mb-1 text-[11px] font-medium text-slate-500 dark:text-slate-400">Niveau Support</label>
                    <select name="support_level" class="w-full rounded-lg border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 py-2 px-3 text-sm outline-none focus:border-blue-500">
                        <option value="">Tous</option>
                        @foreach($levels as $k => $l)
                            <option value="{{ $k }}" {{ request('support_level') == $k ? 'selected' : '' }}>{{ $l }}</option>
                        @endforeach
                    </select>
                </div>
                <div>
                    ...
                    <input type="date" name="date_from" value="{{ request('date_from') }}"
                           class="w-full rounded-lg border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 py-2 px-3 text-sm outline-none focus:border-blue-500">
                </div>
                <div>
    ...
    <div class="rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 overflow-hidden">
        <div class="flex items-center justify-between px-5 py-3 border-b border-slate-200 dark:border-slate-800">
            <p class="text-sm font-semibold">{{ $tickets->total() }} ticket{{ $tickets->total() > 1 ? 's' : '' }}</p>
            ...
            <table class="w-full text-sm">
                <thead>
                    <tr class="border-b border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/50">
                        @php $cols = ['id'=>'ID','title'=>'Titre','user'=>'Utilisateur','category'=>'Catégorie','support_level'=>'Level','priority'=>'Priorité','status'=>'Statut','jira_ticket_id'=>'Jira','created_at'=>'Date']; @endphp
                            ...
                            <th class="px-4 py-3 text-left text-[11px] font-semibold uppercase tracking-wide text-slate-500 whitespace-nowrap">
                                <a href="{{ sortUrl($col, $sort ?? '', $direction ?? 'desc') }}"
                                   class="inline-flex items-center gap-1 hover:text-slate-900 dark:hover:text-slate-100 transition">
                                    ...
                                    @if($icon) <span class="text-blue-600">{{ $icon }}</span>
                ...
                <tbody class="divide-y divide-slate-100 dark:divide-slate-800">
                    @forelse($tickets as $t)
                        @php
                            ...
                            $lCfg = $levelConfig[$t->support_level] ?? ['bg'=>'bg-slate-100','text'=>'text-slate-600','label'=>strtoupper($t->support_level??'N1')];
                        @endphp
                        <tr class="hover:bg-slate-50 dark:hover:bg-slate-800/40 transition group">
                            <td class="px-4 py-3 whitespace-nowrap"><span class="text-xs font-mono text-slate-400">#{{ $t->id }}</span></td>
                            ...
                            <td class="px-4 py-3 whitespace-nowrap"><span class="text-xs text-slate-600 dark:text-slate-300">{{ $t->category ?? '—' }}</span></td>
                            <td class="px-4 py-3 whitespace-nowrap">
                                <span class="inline-flex items-center rounded-full px-2.5 py-1 text-[11px] font-medium {{ $lCfg['bg'] }} {{ $lCfg['text'] }}">{{ $lCfg['label'] }}</span>
                            </td>
                            <td class="px-4 py-3 whitespace-nowrap">
                                <span class="inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-[11px] font-medium {{ $pCfg['bg'] }} {{ $pCfg['text'] }}">
                                    <span class="h-1.5 w-1.5 rounded-full {{ $pCfg['dot'] }}"></span>{{ ucfirst($t->priority ?? 'low') }}
                                </span>
                            </td>
                            <td class="px-4 py-3 whitespace-nowrap">
                                <span class="inline-flex items-center rounded-full px-2.5 py-1 text-[11px] font-medium {{ $sCfg['bg'] }} {{ $sCfg['text'] }}">{{ ucfirst(str_replace('_',' ',$t->status ?? 'open')) }}</span>
                            </td>
                            <td class="px-4 py-3 whitespace-nowrap">
                                @if($t->jira_ticket_id)
                                    <span class="inline-flex items-center rounded-full bg-blue-100 dark:bg-blue-950 px-2 py-0.5 text-[11px] font-medium text-blue-700 dark:text-blue-300">{{ $t->jira_ticket_id }}</span>
                                @else <span class="text-xs text-slate-400">—</span> @endif
                            </td>
                            <td class="px-4 py-3 whitespace-nowrap">
                                <span class="text-xs text-slate-500">{{ $t->created_at->format('d/m/Y') }}</span><br>
                                <span class="text-[10px] text-slate-400">{{ $t->created_at->format('H:i') }}</span>
                            </td>
                            <td class="px-4 py-3 whitespace-nowrap">
                                <div class="flex items-center gap-1">
                                    @if($t->is_urgent) <span class="rounded-full bg-red-100 dark:bg-red-950 px-2 py-0.5 text-[10px] font-medium text-red-700">Urgent</span> @endif
                                    @if($t->is_escalated) <span class="rounded-full bg-amber-100 dark:bg-amber-950 px-2 py-0.5 text-[10px] font-medium text-amber-800">Escaladé</span> @endif
                                    @if(!$t->is_urgent && !$t->is_escalated) <span class="text-[11px] text-slate-400">—</span> @endif
                                </div>
                            </td>
                            <td class="px-4 py-3 whitespace-nowrap">
                    ...
                    @empty
                        <tr><td colspan="12" class="px-4 py-16 text-center text-slate-400">
                            <p class="text-sm font-medium">Aucun ticket trouvé</p>
        ...
        @if($tickets->hasPages())
            <div class="border-t border-slate-200 dark:border-slate-800 px-5 py-3">{{ $tickets->links() }}</div>
        @endif
    </div>
</div>
            ...
            <div class="space-y-4">
                ...
                <div>
                    ...
                    <select name="to_level" id="targetLevel" required
                            class="w-full rounded-lg border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-800 px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500/50 focus:border-blue-500 dark:text-slate-200">
                ...
                <div>
                    ...
                    <select name="assigned_to" class="w-full rounded-lg border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-800 px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500/50 focus:border-blue-500 dark:text-slate-200">
                ...
                <div>
                    ...
                    <div class="flex flex-wrap gap-1.5">
                        <button type="button" onclick="setReason('Requires advanced troubleshooting')" class="px-2 py-1 text-[10px] rounded-full border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800 transition">Requires advanced troubleshooting</button>
                        <button type="button" onclick="setReason('Requires network specialist')" class="px-2 py-1 text-[10px] rounded-full border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800 transition">Requires network specialist</button>
                        <button type="button" onclick="setReason('Requires database administrator')" class="px-2 py-1 text-[10px] rounded-full border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800 transition">Requires database administrator</button>
                        <button type="button" onclick="setReason('Requires infrastructure engineer')" class="px-2 py-1 text-[10px] rounded-full border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800 transition">Requires infrastructure engineer</button>
                        <button type="button" onclick="setReason('Security investigation required')" class="px-2 py-1 text-[10px] rounded-full border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800 transition">Security investigation required</button>
                    </div>
                </div>
            </div>
...
function openEscalateModal(ticketId, currentLevel) {
    document.getElementById('escalateForm').action = '/equipeIT/ui/tickets/' + ticketId + '/escalate';
...
function setReason(reason) { document.getElementById('escalateReason').value = reason; }
