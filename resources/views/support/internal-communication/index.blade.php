...
<div class="space-y-5">
    ...
    <div class="rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 overflow-hidden shadow-sm">
        <div class="grid grid-cols-1 lg:grid-cols-[260px_minmax(0,1fr)_260px] h-[650px] divide-y lg:divide-y-0 lg:divide-x divide-slate-250 dark:divide-slate-800">
            ...
            {{-- COLONNE A : Liste de l'administration (Admins, Managers) --}}
            <div class="flex flex-col overflow-hidden bg-slate-50/20 dark:bg-slate-900/10">
                ...
                <div class="flex-1 overflow-y-auto divide-y divide-slate-100 dark:divide-slate-800/60 bg-white dark:bg-slate-900">
                    @forelse($teamMembers as $member)
                        ...
                        @endphp
                        <a href="{{ route('support.ui.internal.index', ['with' => $member->id]) }}"
                           class="flex items-center gap-3 p-3.5 transition hover:bg-slate-55 dark:hover:bg-slate-800/40 relative border-l-4 
                                  {{ $isActive ? 'bg-blue-600/5 dark:bg-blue-600/10 border-blue-600' : 'border-transparent' }}">
                            ...
                            <div class="min-w-0 flex-1">
                                <div class="flex justify-between items-center">
                                    ...
                                    @if($unread > 0)
                                        <span class="rounded-full bg-blue-600 text-[10px] font-bold text-white px-1.5 py-0.5">{{ $unread }}</span>
                                    @endif
                                </div>
                                <p class="truncate text-[10px] text-slate-455 dark:text-slate-500 font-medium">{{ $member->email }}</p>
                            </div>
                        </a>
                    @empty
            ...
            <div class="flex flex-col overflow-hidden bg-slate-50/30 dark:bg-slate-950/20">
                @if($selectedUser)
                                ...
                                <div class="max-w-[75%] space-y-1">
                                    <div class="flex items-center gap-2 px-1 {{ $isMe ? 'justify-end' : '' }}">
                                        <span class="text-[9px] font-bold uppercase tracking-wider text-slate-400">
                                            {{ $isMe ? 'Vous (Support)' : $msg->sender->name }}
                                        </span>
                                    ...
                                    <div class="rounded-2xl px-4 py-3 text-sm leading-relaxed shadow-sm relative 
                                          {{ $isMe ? 'bg-[#4F6EF7] text-white rounded-tr-none' : 'bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-900 dark:text-slate-100 rounded-tl-none' }}">
                                        ...
                                        @if($msg->ticket)
                                            <div class="mt-2.5 pt-2.5 border-t border-black/10 dark:border-white/10 flex flex-col gap-1.5">
                                                ...
                                                <a href="{{ route('support.ui.tickets.show', $msg->ticket) }}" class="inline-flex items-center justify-center bg-slate-950 dark:bg-slate-150 hover:bg-slate-850 dark:hover:bg-white text-white dark:text-slate-950 text-[10px] font-bold py-1 px-3.5 rounded-lg shadow-sm">
                                                    Ouvrir le ticket
                                                </a>
                                            </div>
                                        @endif
                    ...
                    <div class="border-t border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-4 shrink-0">
                        <form method="POST" action="{{ route('support.ui.internal.send') }}" id="internalSendForm">
                            @csrf
            ...
            {{-- COLONNE C : Fiche Administration --}}
                    ...
                    <div class="p-4 border-b border-slate-200 dark:border-slate-800">
                        ...
                        <dl class="space-y-2.5 text-xs">
                            ...
                            <div>
                                ...
                                <dd class="font-bold text-blue-600 dark:text-blue-400 truncate">{{ $selectedUser->email }}</dd>
                            </div>
                            <div>
                                <dt class="text-slate-450 font-semibold mb-0.5">Rôles</dt>
                                <dd class="mt-1 flex flex-wrap gap-1">
                                    @foreach($selectedUser->roles as $role)
                                        <span class="inline-flex px-2 py-0.5 text-[10px] font-bold bg-blue-500/10 text-[#4F6EF7] rounded-full border border-blue-500/20">{{ $role->name }}</span>
                                    @endforeach
                                </dd>
                            </div>
                        </dl>
