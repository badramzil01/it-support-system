...
<div class="mb-5">
    <div class="mb-3 flex items-center justify-between">
        <div>
            ...
            <p class="text-xs text-slate-500 dark:text-slate-400">Vue globale · {{ now()->format('d/m/Y H:i') }}</p>
        </div>
        <form method="GET" action="{{ route('support.ui.conversations.index') }}" class="relative">
            ...
            <input name="search" value="{{ $search ?? '' }}" placeholder="Rechercher un utilisateur…"
    ...
    <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
        <div class="rounded-xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-4">
            ...
            <p class="mt-1 text-2xl font-semibold">{{ $globalStats['users'] }}</p>
        </div>
        <div class="rounded-xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-4">
            <p class="text-[11px] text-slate-500 dark:text-slate-400">Conversations</p>
            <p class="mt-1 text-2xl font-semibold">{{ $globalStats['convs'] }}</p>
        </div>
        <div class="rounded-xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-4">
            <p class="text-[11px] text-slate-500 dark:text-slate-400">Tickets total</p>
            <p class="mt-1 text-2xl font-semibold">{{ $globalStats['tickets'] }}</p>
        </div>
        <div class="rounded-xl bg-red-50 dark:bg-red-950/40 border border-red-100 dark:border-red-900 p-4">
            ...
            <p class="mt-1 text-2xl font-semibold text-red-700 dark:text-red-300">{{ $globalStats['urgent'] }}</p>
        </div>
        <div class="rounded-xl bg-amber-50 dark:bg-amber-950/40 border border-amber-100 dark:border-amber-900 p-4">
            ...
            <p class="mt-1 text-2xl font-semibold text-amber-800 dark:text-amber-300">{{ $globalStats['escalated'] }}</p>
        </div>
        <div class="rounded-xl bg-blue-50 dark:bg-blue-950/40 border border-blue-100 dark:border-blue-900 p-4">
            ...
            <p class="mt-1 text-2xl font-semibold text-blue-800 dark:text-blue-300">{{ $globalStats['unread'] }}</p>
        </div>
    </div>
</div>

{{-- ══════════════════════════════════════════════
     SECTION 2 — GRILLE UTILISATEURS
══════════════════════════════════════════════ --}}
<div class="mb-5">
    <h3 class="mb-3 text-xs font-semibold uppercase tracking-wide text-slate-500 dark:text-slate-400">
        Utilisateurs — {{ $users->count() }} résultats
    </h3>
                    ...
                    <div class="flex items-start gap-3">
                        <div class="relative shrink-0">
                            ...
                            @if(($user->unread_messages_count ?? 0) > 0)
                                <span class="absolute -top-0.5 -right-0.5 flex h-4 w-4 items-center justify-center rounded-full bg-blue-600 text-[9px] font-bold text-white">{{ min($user->unread_messages_count, 9) }}</span>
                            @endif
                        </div>
                        <div class="min-w-0 flex-1">
                            ...
                            @if($lastMsg)
                                <p class="mt-1 line-clamp-1 text-[11px] text-slate-600 dark:text-slate-300">{{ $lastMsg->content }}</p>
                            @endif
                        </div>
                        <span class="shrink-0 text-[10px] text-slate-400">{{ $lastMsg?->created_at?->diffForHumans(null, true) }}</span>
                    </div>
                    <div class="mt-3 flex flex-wrap gap-1.5">
                        <span class="rounded-full bg-slate-100 dark:bg-slate-800 px-2 py-0.5 text-[10px] font-medium text-slate-600 dark:text-slate-300">{{ $user->conversations_count ?? 0 }} conv.</span>
                        <span class="rounded-full bg-slate-100 dark:bg-slate-800 px-2 py-0.5 text-[10px] font-medium text-slate-600 dark:text-slate-300">{{ $user->tickets_count ?? 0 }} tickets</span>
...
{{-- ══════════════════════════════════════════════
     SECTION 3 — PANNEAU CHAT (masqué par défaut, affiché via JS)
══════════════════════════════════════════════ --}}
<div id="chatPanel" class="{{ $selectedUser ? '' : 'hidden' }} rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 overflow-hidden">
    ...
    <div id="panelHeader" class="flex items-center justify-between border-b border-slate-200 dark:border-slate-800 px-5 py-4 bg-slate-50 dark:bg-slate-800/50">
        <div class="flex items-center gap-3">
            ...
            <div>
                <p id="panelName" class="text-sm font-semibold">{{ $selectedUser?->name }}</p>
                <p id="panelEmail" class="text-xs text-slate-500 dark:text-slate-400">{{ $selectedUser?->email }}</p>
            </div>
        </div>
        <div class="flex items-center gap-3">
            <div id="panelBadges" class="flex flex-wrap gap-1.5">
                @if($selectedUser)
                    <span class="rounded-full bg-slate-100 dark:bg-slate-700 px-2.5 py-1 text-[11px] font-medium text-slate-600 dark:text-slate-300">{{ $tickets->count() }} tickets</span>
                    @if($tickets->where('is_urgent', 1)->count())
                        <span class="rounded-full bg-red-100 dark:bg-red-950 px-2.5 py-1 text-[11px] font-medium text-red-700 dark:text-red-300">{{ $tickets->where('is_urgent',1)->count() }} urgents</span>
                    @endif
                    @if($tickets->where('is_escalated', 1)->count())
                        <span class="rounded-full bg-amber-100 dark:bg-amber-950 px-2.5 py-1 text-[11px] font-medium text-amber-800 dark:text-amber-300">{{ $tickets->where('is_escalated',1)->count() }} escaladés</span>
                    @endif
                @endif
            </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-[240px_minmax(0,1fr)_260px]" style="height: 600px;">
        ...
        <div class="flex flex-col border-b lg:border-b-0 lg:border-r border-slate-200 dark:border-slate-800 overflow-hidden">
            <div class="flex items-center justify-between px-4 py-3 border-b border-slate-200 dark:border-slate-800">
                ...
                @if($selectedUser)
                    <span class="text-[10px] text-slate-400">{{ $conversationsList->count() }}</span>
                @endif
            </div>
                ...
                @if($selectedUser)
                    @forelse($conversationsList as $conv)
                        ...
                        <button type="button"
                            ...
                            <p class="truncate text-xs font-semibold">{{ $conv->title ?: 'Conversation #'.$conv->id }}</p>
                            <p class="mt-0.5 text-[10px] text-slate-400">{{ $conv->latestMessage?->created_at?->format('d/m H:i') ?? $conv->updated_at?->format('d/m H:i') }}</p>
                            <p class="mt-0.5 line-clamp-1 text-[10px] text-slate-500 dark:text-slate-400">{{ $conv->latestMessage?->content }}</p>
                            <div class="mt-1 flex gap-1">
                                <span class="rounded-full bg-slate-100 dark:bg-slate-700 px-1.5 py-0.5 text-[9px] text-slate-500 dark:text-slate-300">{{ $conv->messages_count }} msg</span>
        ...
        <div class="flex flex-col overflow-hidden">
            ...
            {{-- Empty state --}}
            <div id="chatEmpty" class="{{ $selectedConversation ? 'hidden' : 'flex' }} flex-1 flex-col items-center justify-center gap-2 text-center p-8">
            ...
            <div id="chatContent" class="{{ $selectedConversation ? 'flex' : 'hidden' }} flex-col flex-1 overflow-hidden">
                ...
                <div id="chatHeader" class="flex items-center justify-between border-b border-slate-200 dark:border-slate-800 px-4 py-2.5">
                    ...
                    <div class="flex items-center gap-2">
                        @if($selectedConversation && !$ticketForActions)
                            <form method="POST" action="{{ route('support.ui.conversations.tickets.create') }}">
                                @csrf
                        ...
                        @elseif($ticketForActions)
                            <a href="{{ route('support.ui.tickets.show', $ticketForActions) }}" class="inline-flex items-center gap-1 rounded-lg border border-slate-200 dark:border-slate-700 px-2.5 py-1 text-[11px] font-medium hover:bg-slate-50 dark:hover:bg-slate-800 transition">
                ...
                {{-- Escalation banner --}}
                @if($triggerTicket && $triggerMessage)
                    <div class="border-b border-amber-200 dark:border-amber-900 bg-amber-50 dark:bg-amber-950/40 px-4 py-2">
                        <div class="flex items-start gap-2">
                            ...
                            <div class="min-w-0">
                                <p class="text-[11px] font-semibold text-amber-900 dark:text-amber-100">Message déclencheur · {{ $triggerTicket->priority }} · {{ $triggerTicket->category }}</p>
                                <p class="text-[11px] text-amber-800 dark:text-amber-200 italic truncate">"{{ Str::limit($triggerMessage->content, 100) }}"</p>
                            </div>
                        </div>
                    </div>
                @endif
                ...
                <div id="chatBox" class="flex-1 overflow-y-auto bg-slate-50 dark:bg-slate-950 p-4 space-y-3">
                    @if($selectedConversation)
                        @foreach($messages as $message)
                            ...
                            @endphp
                            <div class="flex items-end gap-2 {{ $isRight ? 'justify-end' : 'justify-start' }}" data-message-id="{{ $message->id }}">
                                @if(!$isRight)
                                ...
                                <div class="max-w-[78%]">
                                    <div class="mb-0.5 flex items-center gap-1.5 {{ $isRight ? 'justify-end' : '' }}">
                                        <span class="text-[10px] font-semibold uppercase tracking-wide text-slate-400">
                                            {{ $sender === 'user' ? $selectedUser->name : ($isAi ? 'IA' : 'Support') }}
                                        </span>
                                        <span class="text-[10px] text-slate-400">{{ $message->created_at->format('d/m H:i') }}</span>
                                    </div>
                                    <div class="rounded-2xl px-3.5 py-2 text-sm leading-relaxed {{ $bubble }} {{ $isRight ? 'rounded-br-sm' : 'rounded-bl-sm' }}">
                ...
                <div class="border-t border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-3">
                    @if($errors->any())
                        <div class="mb-2 rounded-lg border border-red-200 dark:border-red-900 bg-red-50 dark:bg-red-950 px-3 py-1.5 text-xs text-red-700 dark:text-red-300">{{ $errors->first() }}</div>
                    @endif
                    <form method="POST" action="{{ route('support.ui.conversations.send') }}" id="replyForm">
                        @csrf
                        ...
                        <textarea name="content" id="replyContent" rows="2" required
                                  ...
                                  placeholder="Écrire une réponse…">{{ old('content') }}</textarea>
            ...
            @if($selectedUser)
                ...
                <div class="border-b border-slate-200 dark:border-slate-800 p-4">
                    ...
                    <div class="grid grid-cols-2 gap-2">
                        <div class="rounded-lg bg-slate-50 dark:bg-slate-800 p-2.5">
                            ...
                            <p class="text-xl font-semibold">{{ $customerStats['conversations'] }}</p>
                        </div>
                        <div class="rounded-lg bg-slate-50 dark:bg-slate-800 p-2.5">
                            <p class="text-[10px] text-slate-500">Tickets</p>
                            <p class="text-xl font-semibold">{{ $customerStats['tickets'] }}</p>
                        </div>
                        <div class="rounded-lg bg-red-50 dark:bg-red-950/40 p-2.5">
                            <p class="text-[10px] text-red-600 dark:text-red-400">Urgents</p>
                            <p class="text-xl font-semibold text-red-700 dark:text-red-300">{{ $customerStats['urgent'] }}</p>
                        </div>
                        <div class="rounded-lg bg-amber-50 dark:bg-amber-950/40 p-2.5">
                            ...
                            <p class="text-xl font-semibold text-amber-800 dark:text-amber-300">{{ $customerStats['escalated'] }}</p>
                        </div>
                    </div>
                </div>
                ...
                <div class="border-b border-slate-200 dark:border-slate-800 p-4">
                    ...
                    <dl class="space-y-2 text-xs">
                        <div class="flex justify-between gap-2">
                            ...
                            <dd class="font-medium truncate">{{ $selectedUser->department ?? $selectedUser->role ?? '—' }}</dd>
                        </div>
                        <div class="flex justify-between gap-2">
                            <dt class="text-slate-500">Inscription</dt>
                            <dd class="font-medium">{{ $selectedUser->created_at?->format('d/m/Y') }}</dd>
                        </div>
                        <div class="flex justify-between gap-2">
                            <dt class="text-slate-500">Email</dt>
                            <dd class="font-medium truncate text-blue-600 dark:text-blue-400">{{ $selectedUser->email }}</dd>
                        </div>
                    </dl>
                </div>
                ...
                <div class="border-b border-slate-200 dark:border-slate-800 p-4">
                    ...
                    @if($lastTicket)
                        <div class="rounded-lg border border-slate-200 dark:border-slate-700 p-3">
                            <p class="text-xs font-semibold">#{{ $lastTicket->id }} · {{ Str::limit($lastTicket->title ?: 'Ticket support', 35) }}</p>
                            <p class="mt-1 text-[10px] text-slate-500">{{ $lastTicket->created_at?->format('d/m/Y H:i') }}</p>
                            <div class="mt-2 flex flex-wrap gap-1">
                                <span class="rounded-full bg-slate-100 dark:bg-slate-700 px-2 py-0.5 text-[9px] text-slate-600 dark:text-slate-300">{{ ucfirst($lastTicket->status) }}</span>
                                <span class="rounded-full bg-slate-100 dark:bg-slate-700 px-2 py-0.5 text-[9px] text-slate-600 dark:text-slate-300">{{ ucfirst($lastTicket->priority ?: 'medium') }}</span>
                                @if($lastTicket->jira_ticket_id)
                                    <span class="rounded-full bg-blue-100 dark:bg-blue-950 px-2 py-0.5 text-[9px] font-medium text-blue-700 dark:text-blue-300">{{ $lastTicket->jira_ticket_id }}</span>
                                @endif
                            </div>
                        </div>
                    @else
                ...
                <div class="p-4">
                    ...
                    <div class="space-y-2">
                        ...
                        @endif
                        @if($ticketForActions)
                            <form method="POST" action="{{ route('support.ui.conversations.tickets.assign', $ticketForActions) }}">
                                @csrf
                                <button class="flex w-full items-center justify-center gap-1.5 rounded-lg border border-slate-200 dark:border-slate-700 px-3 py-2 text-xs font-medium hover:bg-slate-50 dark:hover:bg-slate-800 transition">
                            ...
                            <form method="POST" action="{{ route('support.ui.conversations.tickets.escalate', $ticketForActions) }}">
                                @csrf
                            ...
                            <a href="{{ route('support.ui.tickets.show', $ticketForActions) }}" class="flex w-full items-center justify-center gap-1.5 rounded-lg border border-slate-200 dark:border-slate-700 px-3 py-2 text-xs font-medium hover:bg-slate-50 dark:hover:bg-slate-800 transition">
                                Voir ticket associé
                            </a>
                            @if($ticketForActions->jira_ticket_id)
                                <a href="{{ rtrim(config('services.jira.url','#'), '/').'/browse/'.$ticketForActions->jira_ticket_id }}" target="_blank" class="flex w-full items-center justify-center gap-1.5 rounded-lg bg-blue-600 px-3 py-2 text-xs font-medium text-white hover:bg-blue-700 transition">
