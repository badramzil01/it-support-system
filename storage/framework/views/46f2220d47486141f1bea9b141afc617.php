<header class="sticky top-0 z-10 flex items-center gap-3 px-4 md:px-6 bg-white/80 dark:bg-[#0D0F14]/90 backdrop-blur-md border-b border-slate-200 dark:border-slate-800/80" style="height: var(--header-h);">
    ...
    <div class="flex items-center gap-2 min-w-0">
        ...
        <h1 class="brand-font text-sm font-semibold text-slate-900 dark:text-slate-100 truncate"><?php echo $__env->yieldContent('title', 'Dashboard'); ?></h1>
    </div>
    ...
    <div class="ml-auto flex items-center gap-1.5">
        ...
        <?php endif; ?>
        <?php if(session('info')): ?>
        <div class="hidden sm:flex items-center gap-1.5 rounded-lg border border-blue-200 dark:border-blue-900/60 bg-blue-50 dark:bg-blue-950/50 px-3 py-1.5 text-xs text-blue-700 dark:text-blue-400 font-medium">
            <svg class="h-3.5 w-3.5 shrink-0" viewBox="0 0 20 20" fill="currentColor"><path fill-rule="evenodd" d="M18 10a8 8 0 1 1-16 0 8 8 0 0 1 16 0Zm-7-4a1 1 0 1 1-2 0 1 1 0 0 1 2 0ZM9 9a.75.75 0 0 0 0 1.5h.253a.25.25 0 0 1 .244.304l-.459 2.066A1.75 1.75 0 0 0 10.747 15H11a.75.75 0 0 0 0-1.5h-.253a.25.25 0 0 1-.244-.304l.459-2.066A1.75 1.75 0 0 0 9.253 9H9Z" clip-rule="evenodd"/></svg>
        ...
        <div class="relative">
            <button onclick="toggleDropdown('notifMenu')" class="relative p-1.5 rounded-lg text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-800 transition">
                ...
                ?>
                <span id="notifBadge" class="absolute -top-1 -right-1 inline-flex items-center justify-center h-5 min-w-[20px] px-1 text-[11px] font-semibold rounded-full <?php echo e($unreadCount > 0 ? 'bg-red-500 text-white' : 'bg-transparent text-transparent'); ?>"><?php echo e($unreadCount > 0 ? $unreadCount : ''); ?></span>
            </button>
            <div id="notifMenu" class="dropdown-menu absolute right-0 mt-1.5 w-80 rounded-xl bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 shadow-xl shadow-black/10 overflow-hidden">
                ...
                <div class="px-4 py-2 border-t border-slate-100 dark:border-slate-700">
                    <a href="<?php echo e(route('admin.notifications')); ?>" class="text-xs font-medium text-amber-600 hover:text-amber-700 dark:text-amber-400">Voir tout →</a>
                </div>
            </div>
        </div>
        ...
        <div class="relative pl-1.5 border-l border-slate-200 dark:border-slate-700">
            <button onclick="toggleDropdown('userMenu')" class="flex items-center gap-2 rounded-lg px-2 py-1.5 hover:bg-slate-100 dark:hover:bg-slate-800 transition">
                ...
                <span class="hidden sm:block text-xs font-medium text-slate-700 dark:text-slate-300"><?php echo e(auth()->user()->name ?? 'Admin'); ?></span>
            ...
            <div id="userMenu" class="dropdown-menu absolute right-0 mt-1.5 w-52 rounded-xl bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 shadow-xl shadow-black/10 overflow-hidden py-1">
                <div class="px-3 py-2 border-b border-slate-100 dark:border-slate-700 mb-1">
                    <p class="text-xs font-semibold text-slate-800 dark:text-slate-200"><?php echo e(auth()->user()->name ?? 'Admin'); ?></p>
                    <p class="text-[10px] text-slate-500"><?php echo e(auth()->user()->email ?? ''); ?></p>
                </div>
                <a href="<?php echo e(route('admin.ui.settings.index')); ?>" class="flex items-center gap-2.5 px-3 py-2 text-xs text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700/60 transition">
...
<?php /**PATH C:\Users\badr\Desktop\support it\support-system\resources\views/admin/layouts/partials/header.blade.php ENDPATH**/ ?>