            ...
            <div class="flex flex-col overflow-hidden bg-slate-50/20 dark:bg-slate-900/10">
                ...
                <div class="flex-1 overflow-y-auto divide-y divide-slate-100 dark:divide-slate-800/60 bg-white dark:bg-slate-900">
                    <?php $__empty_1 = true; $__currentLoopData = $teamMembers; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $member): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                        ...
                        ?>
                        <a href="<?php echo e(route('admin.ui.internal.index', ['with' => $member->id])); ?>"
                           class="flex items-center gap-3 p-3.5 transition hover:bg-slate-55 dark:hover:bg-slate-800/40 relative border-l-4 
                                  <?php echo e($isActive ? 'bg-amber-500/5 dark:bg-amber-500/10 border-amber-500' : 'border-transparent'); ?>">
                            ...
                            <div class="min-w-0 flex-1">
                                <div class="flex justify-between items-center">
                                    ...
                                    <?php if($unread > 0): ?>
                                        <span class="rounded-full bg-amber-500 text-[10px] font-bold text-slate-950 px-1.5 py-0.5"><?php echo e($unread); ?></span>
                                    <?php endif; ?>
                                </div>
                                <p class="truncate text-[10px] text-slate-450 dark:text-slate-500 font-medium"><?php echo e($member->email); ?></p>
                            </div>
                        </a>
            ...
            <div class="flex flex-col overflow-hidden bg-slate-50/30 dark:bg-slate-950/20">
                <?php if($selectedUser): ?>
                                ...
                                <div class="max-w-[75%] space-y-1">
                                    <div class="flex items-center gap-2 px-1 <?php echo e($isMe ? 'justify-end' : ''); ?>">
                                        <span class="text-[9px] font-bold uppercase tracking-wider text-slate-400">
                                            <?php echo e($isMe ? 'Vous (Admin)' : $msg->sender->name); ?>
                                    ...
                                    <div class="rounded-2xl px-4 py-3 text-sm leading-relaxed shadow-sm relative 
                                          <?php echo e($isMe ? 'bg-amber-600 text-white rounded-tr-none' : 'bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-900 dark:text-slate-100 rounded-tl-none'); ?>">
                                            ...
                                            <div class="mt-2.5 pt-2.5 border-t border-black/10 dark:border-white/10 flex flex-col gap-1.5">
                                                <div class="rounded-xl bg-slate-100/50 dark:bg-slate-900/50 p-2.5 border border-black/5 dark:border-white/5 text-xs text-slate-650 dark:text-slate-350">
                                                    <div class="flex justify-between items-center">
                                                        ...
                                                        <span class="text-[9px] font-extrabold tracking-wide uppercase px-1.5 py-0.5 rounded bg-slate-200 dark:bg-slate-800 text-slate-600 dark:text-slate-400"><?php echo e($msg->ticket->status); ?></span>
                                                    </div>
                                                    <p class="mt-1 font-semibold truncate"><?php echo e($msg->ticket->title); ?></p>
                                                </div>
                                                <a href="<?php echo e(route('admin.ui.tickets.show', $msg->ticket)); ?>" class="inline-flex items-center justify-center bg-slate-950 dark:bg-slate-150 hover:bg-slate-850 dark:hover:bg-white text-white dark:text-slate-950 text-[10px] font-bold py-1 px-3.5 rounded-lg shadow-sm">
                                                    Ouvrir le ticket
                                                </a>
                                            </div>
                                        <?php endif; ?>
                    ...
                    <div class="border-t border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-4 shrink-0">
                        <form method="POST" action="<?php echo e(route('admin.ui.internal.send')); ?>" id="internalSendForm">
                            <?php echo csrf_field(); ?>
                ...
                <?php if($selectedUser): ?>
                            ...
                            <div>
                                ...
                                <dd class="mt-1 flex flex-wrap gap-1">
                                    <?php $__currentLoopData = $selectedUser->roles; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $role): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                        <span class="inline-flex px-2 py-0.5 text-[10px] font-bold bg-amber-500/10 text-amber-600 rounded-full border border-amber-500/20"><?php echo e($role->name); ?></span>
                                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                                </dd>
                            </div>
                        </dl>
                    ...
                    <div class="p-4">
                        ...
                        <div class="space-y-2.5 max-h-80 overflow-y-auto">
                            <?php $__empty_1 = true; $__currentLoopData = $assignedTickets; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $t): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                                <div class="rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-850/60 p-3 shadow-sm relative group/item">
                                    ...
                                    <p class="text-[11px] font-semibold text-slate-700 dark:text-slate-350 mt-1 line-clamp-2"><?php echo e($t->title); ?></p>
                                    <a href="<?php echo e(route('admin.ui.tickets.show', $t)); ?>" class="block w-full text-center bg-slate-950 dark:bg-slate-100 hover:bg-slate-850 dark:hover:bg-white text-white dark:text-slate-950 font-bold py-1 rounded-lg text-[10px] shadow-sm mt-2.5">
                                        Ouvrir le ticket
                                    </a>
                                </div>
                            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); if ($__empty_1): ?>
...
<?php echo $__env->make('admin.layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xamp2\htdocs\prj\pfe-laravel\it-support-system\resources\views/admin/internal-communication/index.blade.php ENDPATH**/ ?>