    ...
    <div class="bg-white rounded-lg shadow-sm border border-slate-100 p-4">
        ...
        <div id="notifList" class="divide-y divide-slate-100">
            <?php $__empty_1 = true; $__currentLoopData = $notifications; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $n): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                <?php if(!empty($useLaravel) && $useLaravel): ?>
                    <?php
                ...
                <div class="p-4 flex items-start gap-4 hover:bg-slate-50 transition" data-notif-id="<?php echo e($n->id); ?>">
                    ...
                    <div class="flex-1 min-w-0">
                        <div class="flex items-center justify-between">
                            <div class="flex items-center gap-3">
                                <div class="text-sm font-medium truncate"><?php echo e($title); ?></div>
                                <?php if(! $read): ?>
                            ...
                            <div class="text-xs text-slate-400"><?php echo e($time->diffForHumans()); ?></div>
                        </div>
                        <?php if(!empty($body)): ?>
                            <div class="text-sm text-slate-600 mt-2 truncate"><?php echo e(\Illuminate\Support\Str::limit($body, 240)); ?></div>
                        <?php endif; ?>
                    </div>
                    <div class="flex-shrink-0 flex flex-col items-end gap-2">
                        <div class="flex gap-2">
                            <?php if(! $read): ?>
                                <button class="mark-read-btn text-xs px-3 py-1 rounded bg-amber-50 text-amber-700 hover:bg-amber-100" data-id="<?php echo e($n->id); ?>">Marquer lu</button>
                            <?php endif; ?>
                            <form method="POST" action="<?php echo e(route('support.ui.notifications.destroy', $n->id)); ?>" onsubmit="return confirm('Supprimer cette notification ?');">
                                <?php echo csrf_field(); ?>
    ...
    (function(){
        ...
        document.querySelectorAll('.mark-read-btn').forEach(btn => {
            btn.addEventListener('click', async (e) => {
                e.preventDefault();
                const id = e.currentTarget.dataset.id;                const res = await fetch(`<?php echo e(url('equipeIT/ui/notifications')); ?>/${id}/read`, { method: 'POST', headers: { 'X-CSRF-TOKEN': csrf, 'Accept': 'application/json' } });
                if (res.ok) {
        ...
        document.getElementById('markAllReadBtn')?.addEventListener('click', async () => {
            ...
            const res = await fetch(`<?php echo e(url('equipeIT/ui/notifications/read-all')); ?>`, { method: 'POST', headers: { 'X-CSRF-TOKEN': csrf, 'Accept': 'application/json' } });
            if (res.ok) {
        ...
        document.getElementById('bulkDeleteBtn')?.addEventListener('click', async () => {
            ...
            const res = await fetch(`<?php echo e(url('equipeIT/ui/notifications')); ?>`, { method: 'DELETE', headers: { 'X-CSRF-TOKEN': csrf, 'Accept': 'application/json' } });
...
<?php echo $__env->make('admin.layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xamp2\htdocs\prj\pfe-laravel\it-support-system\resources\views/admin/notifications/index.blade.php ENDPATH**/ ?>